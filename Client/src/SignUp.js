        ...
        <Formik
          ...
          onSubmit={async (values, { setSubmitting, setStatus, resetForm }) => {
            ...
            try {
              ...
              const response = await axios.post('http://localhost:3001/api/teachers/register', values);
              setStatus({ success: response.data.message }); // Set success message
              ...
              // Handle error
              if (error.response) {
                setStatus({ error: error.response.data.message }); // Use API error message
              } else {
                setStatus({ error: 'Registration failed. Please try again.' }); // Handle unexpected errors
              }
            } finally {
              setSubmitting(false);
            }
